...
def evaluate(fold, model_name, target = "", use_checkpoint = False, 
						 model_not_named_target = True, imagery_path = None, imagery_source = None, 
						 mode = 'temporal', model_output_dim = 768, grouped_bands = None):
    ...
    print(f"Evaluating {model_name} on fold {fold} with target {target} using checkpoint {checkpoint if use_checkpoint else 'None'}")
    ...
    normalization = 30000. if imagery_source == 'L' else 3000.
    ...
    available_imagery = [os.path.join(imagery_path, d, f)
    ...
    base_model = torch.hub.load('facebookresearch/dino:main', model_name) if 'dino_' in model_name \
    ...
    transform = transforms.Compose([
        transforms.Resize((transform_dim, transform_dim)),
        transforms.ToTensor()
    ])
